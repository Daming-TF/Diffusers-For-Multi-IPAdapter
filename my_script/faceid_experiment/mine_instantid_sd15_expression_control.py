...
def inference_instantid(checkpoint_dir, ckpt_name, resampler=True, num_tokens=16, output_dir=None):
    # 1. init
    id_path = "/home/mingjiahui/projects/IpAdapter/IP-Adapter/data/all_test_data/guonan.jpg"
    kps_path = "/home/mingjiahui/projects/IpAdapter/IP-Adapter/data/average_id/guonan.jpg"
    prompt_template= "a young woman with a {} face, wearing a pink shirt. She is standing in front of a fence, possibly in a park or an outdoor setting. The woman appears to be enjoying her time outdoors, possibly engaging in a sport or a recreational activity. "
    expression_keys = [
        'happy', 
        'sad', 
        'ecstatic',
    ...
    assert len(face_info) > 0, f"no face find ==> {id_path}"
    ...
    face_image = transform(face_image)
    face_info = app.get(cv2.cvtColor(np.array(face_image), cv2.COLOR_RGB2BGR))
    assert len(face_info) > 0, f"no face find ==> {kps_path}"
    face_info = sorted(face_info, key=lambda x:(x['bbox'][2]-x['bbox'][0])*(x['bbox'][3]-x['bbox'][1]))[-1]   # only use the maximum face
    ...
    for expression_key in tqdm(expression_keys):
        ...
        # save
        save_name = f"id-{os.path.basename(id_path)}--kps-{os.path.basename(kps_path)}--expression-{expression_key}.jpg"
...
if __name__ == '__main__':
    ...
    parser.add_argument("--control_condition", type=str, required=True, help="Union in ['txt_image', 'image']")     
