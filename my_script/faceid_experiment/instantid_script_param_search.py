...
def main(args):
    id_path = "/home/mingjiahui/projects/IpAdapter/IP-Adapter/data/test/mjh_laughing.jpg"
    ...
    control_weights = np.arange(0, 1+0.25, 0.25).tolist()
    ip_scales = np.arange(0, 1.0+0.25, 0.25).tolist()
    ...
    for control_weight in control_weights:
        ...
        save_path = os.path.join(args.save_dir, f"param_search--id-{os.path.basename(id_path).split('.')[0]}--kps-{os.path.basename(kps_path).split('.')[0]}.jpg")
