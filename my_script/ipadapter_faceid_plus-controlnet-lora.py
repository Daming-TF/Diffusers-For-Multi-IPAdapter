...
def main(args):
    ...
    image = transform(Image.open(args.input).convert("RGB"))
    hed_map = get_control_image(args.control_mode, image)
    hed_map.save(os.path.join(args.output, os.path.basename(args.input).split('.')[0]+'_hed.jpg'))
    ...
    images = ip_model.generate(
        ...
        num_samples=4, num_inference_steps=30, seed=42, guidance_scale=6,
    ...
    save_name += f"-lora_{args.lora}_{args.lora_weight}"
    save_name += f"-faceid_lora_{args.faceid_lora_weight}"
    save_name += f"-s_scale_{args.s_scale}"
    save_name += f"-ControlNet_weight_{args.control_scale}"
