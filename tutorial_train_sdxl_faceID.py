...
def main():
    ...
    accelerator = Accelerator(
        ...
        # kwargs_handlers=[DistributedDataParallelKwargs(find_unused_parameters=True)],       # JIAHUI'S MODIFY
    )
    ...
    vae.to(accelerator.device) # use fp32            # The VAE is in float32 to avoid NaN losses.
    text_encoder.to(accelerator.device, dtype=weight_dtype)
    text_encoder_2.to(accelerator.device, dtype=weight_dtype)
    # image_encoder.to(accelerator.device, dtype=weight_dtype)


    # 5.set LoRA        JIAHUI'S MODIFY
    # now we will add new LoRA weights to the attention layers
    # Set correct lora layers ---only train unet
    # unet_lora_config = LoraConfig(
    #     r=args.rank, init_lora_weights="gaussian", target_modules=["to_k", "to_q", "to_v", "to_out.0"]
    # ) 
    # unet.add_adapter(unet_lora_config)

    # The following is different from the diffusers lora training strategy, 
    # for diffusser：unet fixed parameter is fp16 and lora  parameter is fp32, 
    # for ipadapter：the entire unet use fp32
    ...
    # )
    # AdamW8bit
    optimizer = torch.optim.AdamW8bit(params_to_opt, lr=args.learning_rate, weight_decay=args.weight_decay, eps=1e-08)
    ...
    # JIAHUI'S MODify
    # unet, ip_adapter, optimizer, train_dataloader = accelerator.prepare(
    #     unet, ip_adapter, optimizer, train_dataloader)
    # +++++++++++++++++++++++++++++++++++
