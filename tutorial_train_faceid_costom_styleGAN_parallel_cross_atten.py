...
import logging
from my_script.deepface.eval_model import distance, inference_styleGAN
from my_script.wplus_adapter.models import WProjModel
import subprocess
if is_torch2_available():
    from ip_adapter.attention_processor import AttnProcessor2_0 as AttnProcessor
    from my_script.wplus_adapter.attention_processor import WPlusAttnProcessor2_1 as WPlusAttnProcessor
else:
    from ip_adapter.attention_processor import AttnProcessor
    from my_script.wplus_adapter.attention_processor import WPlusAttnProcessor
# +++++++++++++++++
...
class MyDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            face_id_embed = torch.load(embeds_path, map_location=torch.device('cpu')).squeeze()
...
class WPlusAdapter(torch.nn.Module):
    ...
    def load_from_checkpoint(self, ckpt_path: str):
        ...
        self.image_proj_model.load_state_dict(state_dict["w_proj"], strict=True)
        self.adapter_modules.load_state_dict(state_dict["wplus_adapter"], strict=True)
...
def main():
    ...
    wplus_adapter = WPlusAdapter(unet, w_proj_model, adapter_modules, args.pretrained_ip_adapter_path)
    ...
    counter = 0
    name_list = [] 
    for name, params in wplus_adapter.named_parameters():
        if params.requires_grad==True:
            name_list.append(name)
    print( len(name_list))
    print(len(list(wplus_adapter.image_proj_model.parameters())))
    print(len(list(wplus_adapter.adapter_modules.parameters())))
    # ++++++++++++++++++++++++++++++
    
    # optimizer
    params_to_opt = itertools.chain(wplus_adapter.image_proj_model.parameters(),  wplus_adapter.adapter_modules.parameters())
    ...
    for epoch in range(0, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if global_step % args.save_steps == 0 and accelerator.is_main_process:
                ...
                sd = accelerator.unwrap_model(wplus_adapter).state_dict()
                result = {
                    'w_proj':{},
                    'wplus_adapter':{},
                }
                for k in sd:
                    if k.startswith("unet"):
                        ...
                        result['w_proj'][k.replace("image_proj_model.", "")] = sd[k]
                    elif k.startswith("adapter_modules"):
                        result['wplus_adapter'][k.replace("adapter_modules.", "")] = sd[k]
                save_path_ = os.path.join(save_path, 'sd15_faceid_wplus.bin')
                accelerator.save(result, save_path_)
                print(f"ckpt has saved in {save_path_}")
                inference_styleGAN(save_path, 'sd15_faceid_wplus.bin', args.sr)
